object Lab3 extends jsy.util.JsyApplication {
  ...
  def eval(env: Env, e: Expr): Expr = {
    ...
    case _ => e2 match{
      ...
      case Binary(Seq, e1, e2) => return {eToVal(e1); eToVal(e2)}
      ...
      case Call(e1, e2) => eToVal(e1) match {
        ...
        case Function(None, v1, v2) => eval(extend(env, v1, eToVal(e2)), v2)

        case Function(Some(v1),v2,v3) =>  eval(extend(extend(env, v1, eval(env, e1)), v2, eval(env, e2)), v3)
  ...
  def substitute(e: Expr, v: Expr, x: String): Expr = {
    ...
    e match {
      ...
      case Binary(bop, e1, e2) => bop match {
        case (Eq|Ne) => e1 match {
          case Function (a, b, c) => throw new DynamicTypeError(e)
          case _ => Binary(bop, subst(e1), subst(e2))
        }
        case _ => Binary(bop, subst(e1), subst(e2))
      }
      case If(e1, e2, e3) => If(subst(e1), subst(e2), subst(e3))
      case ConstDecl(c, e1, e2) => if (c == x) ConstDecl(c, subst(e1), e2) else ConstDecl(c, subst(e1), subst(e2))
      case Call(e1, e2) => Call(subst(e1), subst(e2))
      case Function(p, x2, f) => if (x2 == x || p == Some(x)) Function(p, x2, f) else Function(p, x2, subst(f))      
    ...
    e match {
      ...
      case Binary(bop, v1, v2) if (isValue(v1) && isValue(v2)) => bop match {
        ...
        case Ne => (v1, v2) match {
          ...
          case (v1, v2) => B(toBoolean(v1) != toBoolean(v2))
        }
        case Eq => (v1, v2) match {
          case (Function(_,_,_), v2) => throw new DynamicTypeError(e)
          case (v1, Function(_,_,_)) => throw new DynamicTypeError(e)
        ...
        case Call(Function(funct, x, esh), v2) if (isValue(v2)) => Function(funct, x, esh) match{
          case Function(Some(funct), x, esh) => substitute(substitute(esh, v2, x), Function(Some(funct), x, esh), funct)
          case Function(funct, x, esh) => substitute(esh, v2, x)
        }      
      ...
      case Binary (bop, e1, e2) =>(bop, e1, e2) match{
        ...
        case (bop, v1, e2) => Binary(bop, step(e1), e2)
      }
      ...
      case Call(e1, e2) => (e1, e2) match {
        case (Function(None, x,esh), e2) => Call(Function(None, x,esh), step(e2))
        case (Function(Some(y), x,esh),e2) => Call(Function(Some(y), x,esh), step(e2))
